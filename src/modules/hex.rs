...
pub fn commands<'a, 'b>() -> Vec<Command<'a, 'b>> {
	vec![
		...
		Command {
			app: SubCommand::with_name("h2a").about("Convert hex to byte array").arg(
				Arg::with_name("INPUT")
					.required(false)
					.index(1)),
			f: h2a,
		...
		Command {
			app: SubCommand::with_name("a2h").about("Convert byte array to hex").arg(
				Arg::with_name("INPUT")
					.required(false)
					.index(1)),
			f: a2h,
...
fn h2a(matches: &ArgMatches) -> Result<Vec<String>, String> {
	...
	let result = input.into_iter().map(|x|format!("{}", x)).collect::<Vec<String>>().join(", ");
...
fn a2h(matches: &ArgMatches) -> Result<Vec<String>, String> {
	...
	let input = Regex::new(", *").expect("qed").split(input).collect::<Vec<_>>();
	let input = input.into_iter().map(|x| x.parse::<u8>()).collect::<Result<Vec<_>, _>>().map_err(|_| "Invalid byte array")?;
			...
			("h2a",
			 vec![
				 Case {
					 desc: "".to_string(),
					 input: vec!["0x61626364"].into_iter().map(Into::into).collect(),
					 output: vec!["[97, 98, 99, 100]"].into_iter().map(Into::into).collect(),
					 is_example: true,
					 is_test: true,
